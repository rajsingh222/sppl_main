...
try {
    ...
    $stmt = $pdo->prepare("SELECT sppl_dpc_reports FROM projects WHERE id = ?");
    ...
    $existingReports = json_decode($project['sppl_dpc_reports'], true);
    ...
    $stmtUpdate = $pdo->prepare("UPDATE projects SET sppl_dpc_reports = ? WHERE id = ?");
    $stmtUpdate->execute([
    ...
    echo json_encode([
        ...
        "sppl_dpc_reports" => $mergedReports
    ]);
